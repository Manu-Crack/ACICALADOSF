...
/**
 * POST /api/admin/products/upload — Subir imagen WebP a Supabase Storage (bucket products-images)
 * DELETE /api/admin/products/upload — Eliminar imagen de Storage por su URL o path
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const rawTitle = (formData.get("title") as string | null) || "producto";
    ...
    // Subir obligatoriamente como image/webp al bucket products-images en Supabase Storage
    ...
    if (uploadError) {
      console.error("Upload error en Supabase Storage (products-images):", uploadError);
      return NextResponse.json(
    ...
    console.error("Products upload error:", errorMsg);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const storagePath = extractStoragePath(url, "products-images");
    if (!storagePath) {
    ...
    if (removeError) {
      console.error("Error al remover de Storage (products-images):", removeError);
