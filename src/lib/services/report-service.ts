 ...
 * Realiza cálculos exactos respetando las reglas financieras y de exclusión de anulados.
 */
export async function buildFullReportData(
  ...
  let paymentsQuery = supabase
    ...
    .select(`
      ...
      voided_by,
      void_reason,
      bookings (booking_code, client_first_name, client_last_name)
    `)
...
interface RawBookingRow {
  ...
  // ---------------------------------------------------------------------------
  // 6. Mapear Reservas y Calcular Desgloses
  // ---------------------------------------------------------------------------
  ...
  ((rawBookings || []) as unknown as RawBookingRow[]).forEach((b) => {
    ...
    (b.booking_services || []).forEach((bs) => {
      ...
      const sType = bs.services?.type || b.service_type;
  ...
  return {
    ...
    services_breakdown: Object.values(servicesMap).sort((a, b) => b.total_revenue_cents - a.total_revenue_cents),
